...
class HBNBCommand(cmd.Cmd):
    ...
    # str: <key>="<value>"
    # int: <key>=<digit>
    # float: <key>=<digit>.<decimal>
    # (<regex match>, type)
    param_types = {
        "str": (r'\w+="[^\s]+"(?=\s|$)', str),
        "int": (r'\w+=[\+-]?\d+(?=\s|$)', int),
        "float": (r'\w+=[\+-]?\d+\.\d+(?=\s|$)', float)
    }

    def extract_params(self, param_string):
        # General params format: <key>=<value>
        params = {}

        for regex, _type in self.param_types.values():
            matches = re.findall(regex, param_string)

            for match in matches:
                # Format of tuple (<key>, "=", <value>)
                key, _, value = match.partition("=")

                if _type is str:
                    new_value = value[1:-
                                      1].replace('"', '\"').replace("_", " ")
                    params[key] = _type(new_value)
                else:
                    params[key] = _type(value)
    ...
    def do_create(self, args):
        ...
        # Update new instance with params
    ...
    def do_all(self, args):
        ...
        if args:
            ...
            for v in storage.all(models.classes[cls]).values():
